...
data class AddExpr(override val lhs: Expression, override val rhs: Expression): BinaryOperator {
...
data class NumericExpr(val value: Int) : Expression {
    override fun evaluate(random: () -> Int): Face = Face(value, null)
}

data class DiceExpr(val dice: Dice) : Expression {
    override fun evaluate(random: () -> Int): Face {
...
fun List<Expression>.evaluate(genRandom: () -> Int): List<Face> =
    map { it.evaluate(genRandom) }
